...
class TestTemplates:
    ...
    async def test_get_template(self, client, auth_headers):
        ...
        response = await client.get(f"/api/templates/{template_id}", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_template_not_found(self, client, auth_headers):
        ...
        response = await client.get("/api/templates/template_inexistente_xyz", headers=auth_headers)
        assert response.status_code == 404

    async def test_list_templates_unauthorized(self, app):
        """GET /api/templates sem autenticação retorna erro."""
        from app.auth import get_current_user
        app.dependency_overrides.pop(get_current_user, None)
        transport = httpx.ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as c:
            response = await c.get("/api/templates")
